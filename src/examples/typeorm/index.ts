...
export async function seed() {
    const companyValues = [{ name: "company1" }, { name: "company2" }, { name: "company3" }];
    ...
    const [chair1, chair2] = await Promise.all(chairValues.map((v) => globalDataSource.getRepository(Chair).save(v)));
    ...
    const [app1, app2, app3] = await Promise.all(
        appValues.map((v) => globalDataSource.getRepository(ApplicationSoftware).save(v)),
    );
    ...
    const [pc1, pc2, pc3, pc4] = await Promise.all(
        pcValues.map((v) => globalDataSource.getRepository(PersonalComputer).save(v)),
    );
}
...
export async function listen(port: number, resolvers: NonEmptyArray<Function>): Promise<ListenResult> {
    const app = express();
