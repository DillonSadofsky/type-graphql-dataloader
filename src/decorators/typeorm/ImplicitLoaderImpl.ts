...
async function handler<V>(
	...
	const dataloader = container.get<DataLoader<string, V> | DataLoader<string, V[]>>(serviceId);
...
class ToOneOwnerDataloader<V extends ObjectLiteral> extends DataLoader<string, V | null> {
	constructor(relation: RelationMetadata, connection: DataSource) {
		super(async (pks) => {
			...
			const entities = await findEntities<V>(relation, connection, pks, relationName, columns);
...
class OneToManyDataloader<V extends ObjectLiteral> extends DataLoader<string, V[]> {
	constructor(relation: RelationMetadata, connection: DataSource) {
		super(async (pks) => {
			...
			const entities = await findEntities<V>(relation, connection, pks, relation.propertyName, columns);
			const referencedColumnNames = columns.map((c) => c.referencedColumn!.propertyPath);
...
async function findEntities<V extends ObjectLiteral>(
	...
	const qb = connection.createQueryBuilder<V>(relation.type, relation.propertyName);

	if (relation.isOneToOneOwner || relation.isManyToOne) {
		qb.innerJoinAndSelect(`${relation.propertyName}.${relationName}`, relationName);
	} else if (relation.isOneToOneNotOwner || relation.isOneToMany || relation.isManyToMany) {
		const inversePropName = relation.inverseRelation!.propertyName;
