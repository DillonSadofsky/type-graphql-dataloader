...
export function ExplicitLoaderImpl<V extends ObjectLiteral>(
    ...
    return (target: object, propertyKey: string | symbol) => {
        UseMiddleware(async ({ root, context }, next) => {
            ...
            if (option?.selfKey && !(relation.isOneToMany || relation.isOneToOneNotOwner)) {
...
function directLoader<V extends ObjectLiteral>(
    ...
    return async (ids: readonly any[]) => {
        const entities = keyBy(
            await connection.createQueryBuilder<V>(relation.type, relation.propertyName).whereInIds(ids).getMany(),
            grouper,
        ) as Dictionary<V>;
...
class SelfKeyDataloader<V extends ObjectLiteral> extends DataLoader<any, V[]> {
    constructor(relation: RelationMetadata, connection: DataSource, selfKeyFunc: (root: any) => any) {
        super(async (ids) => {
