...
import { Cert, Chair, PersonalComputer, Desk, Employee } from "../examples/typeorm/entities/index.js";
...
type typename = "Company" | "Employee" | "Desk" | "Chair" | "Cert" | "ApplicationSoftware" | "PersonalComputer";

const coalesceTypeNames = (objects: ObjectLiteral[]): typename => {
	const typename = objects.map((a) => a.__typename).reduce((a, b) => (a === b ? a : null));
	if (typename == null) {
		...
		await Promise.all(
			Object.keys(obj).map(async (k) => {
				...
				const getSelfEntity = async () =>
					(await dataSource.getRepository(objectTypes[obj.__typename as typename]).findOneOrFail({
						where: { name: obj.name },
						relations: [k],
					})) as any;
